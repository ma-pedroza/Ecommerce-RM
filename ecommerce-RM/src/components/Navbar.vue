        ...
        <div class="flex-1 max-w-2xl mx-8">
          <div class="relative group">
            ...
            <input
              v-model="search"
              type="text"
              placeholder="Descubra nossos produtos..."
          ...
          <!-- Resultados -->
          <ul v-if="search" class="mt-4 bg-slate-800 rounded-xl text-white p-4 shadow-xl">
            <li
              v-for="produto in produtos"
              :key="produto.id"
              class="py-1 hover:text-rose-400"
            >
              {{ produto.title }}
            </li>
            <li v-if="produtos.length === 0" class="text-slate-400 italic">Nenhum resultado encontrado</li>
          </ul>
        </div>
